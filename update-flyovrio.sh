...
git clone --quiet --depth 1 https://github.com/flyovr-io/flyovrio-update.git
cd flyovrio-update
...
for line in $(grep -v -e '^#' -e '^$' boot-configs/flyovrio-config.txt); do
    if ! grep -qs "$(echo $line | cut -d= -f1)" /boot/flyovrio-config.txt; then
        echo $line >> /boot/flyovrio-config.txt
    fi
done

# make sure things are executable before starting
chmod a+x /usr/local/bin/flyovrio-create-uuid.sh 
chmod a+x /usr/local/bin/update-flyovrio
chmod a+x /usr/local/bin/flyovrio-feed.sh 
chmod a+x /usr/local/bin/pingfail.sh 
chmod a+x /usr/bin/flyovrio-first-run
chmod a+x /usr/bin/flyovrio-temp.sh
chmod a+x /usr/bin/flyovrio-pi-exporter
chmod a+x /usr/bin/flyovrio-978-start
#chmod a+x /usr/bin/flyovrio-978
#chmod a+x /usr/bin/flyovriofeeder
...
# enable services
systemctl enable \
    flyovrio-first-run.service \
    flyovrio-zt-enable.service \
    readsb.service \
    flyovrio-mlat.service \
    flyovrio-feed.service \
    pingfail.service
...
cd $updir
git clone --quiet --depth 1 https://github.com/flyovr-io/readsb.git
...
cp -f readsb /usr/bin/flyovriofeeder
cp -f readsb /usr/bin/flyovrio-978
cp -f readsb /usr/bin/readsb
cp -f viewadsb /usr/bin/viewadsb


echo 'make sure unprivileged users exist (readsb / flyovrio) ......'
for USER in flyovrio readsb; do
...
restartIfEnabled flyovrio-feed
restartIfEnabled flyovrio-978

cd $updir
rm -rf $updir/readsb

VENV=/usr/local/share/flyovrio/venv/
...
; then
    ...
    echo "Please report this error on Github or Discord."
    echo "--------------------"
fi

echo 'starting services .......'
restartIfEnabled flyovrio-mlat
...
echo 'update tar1090 ...........'
bash -c "$(wget -nv -O - https://raw.githubusercontent.com/flyovr-io/tar1090/master/install.sh)"

if [[ -f /boot/flyovrio-config.txt ]]; then
    if ! grep -qs -e 'GRAPHS1090' /boot/flyovrio-config.txt; then
        echo "GRAPHS1090=yes" >> /boot/flyovrio-config.txt
    fi
    if ! grep -qs -e "ZEROTIER" /boot/flyovrio-config.txt; then
        echo "ZEROTIER=no" >> /boot/flyovrio-config.txt
    fi
fi
...
echo "8.2.$(date '+%y%m%d')" > /boot/flyovrio-version-decoder
