...
git clone --quiet --depth 1 https://github.com/tmantti/adsbfi-update.git
cd adsbfi-update
...
if [[ -f /boot/adsbfi-config.txt ]]; then
    ...
    if ! grep -qs -e "ZEROTIER" /boot/adsbfi-config.txt; then
        echo "ZEROTIER=no" >> /boot/adsbfi-config.txt
    fi
fi
